...
:SET_AMI_ID
  SET /P AMI_ID="Set AMI ID : "
  
  IF "%AMI_ID%" == "" GOTO SET_AMI_ID

:SET_INSTANCE_TYPE
  SET /P INSTANCE_TYPE="Set INSTANCE TYPE : "
  
  IF "%INSTANCE_TYPE%" == "" GOTO SET_INSTANCE_TYPE

:SET_KEY_NAME
  SET /P KEY_NAME="Set KEY NAME : "
  
  IF "%KEY_NAME%" == "" GOTO SET_KEY_NAME

:SET_SUBNET_ID
  SET /P SUBNET_ID="Set Subnet Id : "
  
  IF "%SUBNET_ID%" == "" GOTO SET_SUBNET_ID
  
:SET_SG_IDS
  SET /P SG_IDS="Set Security Group Ids : "
  
  IF "%SG_IDS%" == "" GOTO SET_SG_IDS

:SET_PRIV_IP
  SET /P PRIV_IP="Set Private Ip : "
  
  IF "%PRIV_IP%" == "" GOTO SET_PRIV_IP

:SET_ROOT_EBS_SIZE
  SET /P ROOT_EBS_SIZE="Set Root EBS Size(G) : "
  
  IF "%ROOT_EBS_SIZE%" == "" GOTO SET_ROOT_EBS_SIZE

:SET_ROOT_EBS_TYPE
  SET /P ROOT_EBS_TYPE="Set Root EBS Type : "
...
:SET_INSTANCE_TAGS
  ...
  echo.
  SET /P INSTANCE_TAGS="Set Instance Tags : "
...
:SET_VOLUME_TAGS
  ...
  echo Tags Example : {Key=Name, Value=CLI_TEST},{Key=Owner, Value=COMMON}
  echo.
  SET /P VOLUME_TAGS="Set Volume Tags : "
  
  IF "%VOLUME_TAGS%" == "" GOTO SET_VOLUME_TAGS

:SET_PROFILE
  SET /P PROFILE_NAME="Set Profile Name : "
...
echo.
call aws ec2 describe-images --image-ids %AMI_ID% --query "Images[*].{ID:RootDeviceName}" --output text --profile %PROFILE_NAME% > tempRootDeviceName.txt
...
echo Root EBS Size      = %ROOT_EBS_SIZE% G
